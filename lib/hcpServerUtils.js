...
function getStatusNotificationDelay() {
  console.log();
  console.log(Math.floor(Math.random() * (hcpConfig.HCP_DELAY_MAXIMUM - hcpConfig.HCP_DELAY_MINIMUM + 1)) +
        ...
        'identity': {
          ...
          'password': {
            'user': {
              ...
              'password': hcpConfig.HCP_AUTH_USER
            }
          }
        },
        'scope': {
...
function getUpdateContextOptions(request, attributeName, attributeType, attributeValue) {
  var options = {
    ...
    headers: {
      ...
      'fiware-service-path': request.raw.req.headers[hcpConfig.HEADER.FIWARE_SERVICE_PATH],
      'Content-Type': 'application/json',
  ...
  npmRequest(
    ...
    function(error, response, body) {
      if (error || (body && body.contextResponses && Array.isArray(body.contextResponses) && body.contextResponses[0] &&
        ...
        hcpLogger.debug(
          request.hcp.context,
          'Successfully response by the Context Broker to a status notification update request: {' +
            'request: ' + JSON.stringify(options) + ', ' +
  ...
  npmRequest(
    ...
    function(error, response, body) {
      if (error || (body && body.contextResponses && Array.isArray(body.contextResponses) && body.contextResponses[0] &&
        ...
        hcpLogger.debug(
          request.hcp.context,
          'Successfully response by the Context Broker to a command request: {' +
            'request: ' + JSON.stringify(options) + ', ' +
            'response: ' + JSON.stringify(body) + ' ' +
          '}'
        );
      ...
      npmRequest(
        ...
        function(error, response, body) {
          if (error || (body && body.contextResponses && Array.isArray(body.contextResponses) &&
            ...
            hcpLogger.debug(
              request.hcp.context,
              'Successfully response by the Context Broker to a queryContext request: {' +
                'request: ' + JSON.stringify(options) + ', ' +
                'response: ' + JSON.stringify(body) + ' ' +
              '}'
            );
